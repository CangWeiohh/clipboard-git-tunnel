...
帧种类：`peer_probe`（A 启动就绪探针，B 只 ACK）/ `req_meta` / `req_data` / `req_end` / `req_commit` / `resp_begin`（不 ACK）/ `resp_meta` / `resp_data` / `resp_end` / `resp_single` / `req_single` / `ack` / `error`。
...
```text
start_a.bat / start_b.bat   # 顺序任意；A 启动后自动 peer_probe 等待 B/HSR
```

A 只有收到 B 的启动探针 ACK（日志 `peer.ready`）才开放 9999 并打印 `listener.ready`；因此不再需要人工等 B 心跳。若 A 先启动，会自动持续重试，B 后启动后下一次 probe 即握手成功。
...
- 启动顺序任意：A 通过 `peer_probe → ack` 自动等待 B 与 HSR 双向链路；只有 `peer.ready` 后才开放 9999。部署后需保持 A 端 VM/HSRClient 窗口在前台

## 常见坑（按事故历史）

1. 504 五轮排查：焦点丢失（第一轮）、req_commit/resp_meta 屏障缺失（第二轮）、跨请求 write_gap 缺失（第三轮）、OpenClipboard 窗口太短（第四轮）、A/B 混版（第五轮）、HSRClient 窗口失焦导致剪贴板同步整体跳过（第六轮，`req_single` 5 次重写 B 端零接收）。第七轮（resp_begin 空等）：A 端 `http.request.begin` 后日志静止 100s+、剪贴板持续新增同类 QTC1 内容、B 端零日志——根因是传输层全是 DEBUG 日志无法定位 + resp_begin 首写违反同侧写间隔被 HSR 静默丢弃 + B 端屏障无上限饿死新请求。第八轮（B 端启动报错）：`focus.py` `IsWindowVisible` 缺 argtypes，64 位 HWND 按 C int 转换溢出，`Exception ignored` 且 EnumWindows 提前终止——焦点控制是后来给 B 端新加的，A 端同代码同样有隐患，已统一补 argtypes + 回调兜底。第九轮（IDEA 更新 504）：A 端 `req_single` 本地写入 5 次成功但零 ACK、B 端零接收（无 frame.receive）、A 端无任何 focus 警告——A→B 的 HSR 同步整体未发生，焦点控制器 `_hwnd` 无目标时静默返回造成盲区；已补 focus.target/focus.hsr_not_found/focus.hsr_missing 日志 + B 端 15s 心跳。第十轮（拓扑更正）：云桌面 B 端**本来就没有 HSR 客户端**（HSR 只在 A 端环境），B 端 `focus.hsr_not_found` 是预期噪音非故障；真正故障是 A 端 HSR 会话没在同步（窗口在、前台正常、写入成功，但剪贴板没越过）——已撤掉 B 端焦点控制（120f1ab 的错误假设），用 clipboard_bench 验证 HSR 链路。
...
11. **启动就绪要握手，不靠人工看心跳**：A 在开放 9999 前持续发送 `peer_probe`，B 收到只回 ACK；`peer.ready` 同时证明 B 进程、A→B、B→A 三者就绪。A/B 启动顺序任意；A 先启动时使用同 session、递增 retry 重发，B 启动基线只忽略当前 probe，下一次变化必被观察。握手前 Mac/IDEA 连不到 9999，不会先收请求再 25s 504。
...
14. **焦点缺失必须可见**：焦点控制器找不到 HSRClient 窗口（`_hwnd is None`）时旧代码静默返回——A 端本地写入成功（`clipboard.write`）、B 端零接收、无任何警告，与"HSR 通道断了"无法区分。现在：扫描到目标打 INFO `focus.target`（含 hwnd/title）；扫不到打 WARNING `focus.hsr_not_found`（30s 节流，带 `seen_windows` 实际窗口进程列表）；写前仍无目标打 WARNING `focus.hsr_missing`（15s 节流）。ACK 超时 hint 提示检查两端 HSR 状态。
15. **B 端要有心跳**：纯剪贴板传输下"没收到帧"与"B 卡死"在日志上不可区分。b_tunnel 每 15s INFO `process.heartbeat`（启动立即打一拍，uptime_s）。
16. **HSR 只在 A 端环境**：HSR 客户端（cmssonline-receiver 的 hsrclient.exe）只存在于 A 端（VM），云桌面 B 端没有也不应有自己的 HSR 窗口——B 端加焦点控制会永久打出误导性的 `focus.hsr_not_found`。焦点诊断只对 A 端有意义。判读：A 有 `focus.target` 且无 HSR 警告、但 req_single 无 ACK → **A 端 HSR 会话（到云桌面的连接）没在同步**，用 `tools/clipboard_bench.py`（B 端 --role reader、A 端 --role writer）验证，失败则重启/重连 A 端 HSR。
